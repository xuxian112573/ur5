...
class add_box:
    ...
    def _add_table(self, name):
        ...
        q = quaternion_from_euler(0.0, 0.0, numpy.deg2rad(90.0))#numpy.deg2rad(x)：度转换为弧度。
...
if __name__ == '__main__':
    ...
    gold_block_pose = Pose(position=Point(x=0.8, y=0, z=0.74+0.03))
    
    dropbox_block_path = pack_path+os.sep+'urdf'+os.sep+'objects'+os.sep+'dropbox.urdf'
    dropbox_block_name = 'dropbox'
    dropbox_block_pose = Pose(position=Point(x=0.0, y=-0.85, z=0.24+0.03))
    delete_gazebo_model([table_name,fangkuai_name,gold_block_name,dropbox_block_name])
    ...
    spawn_gazebo_model(dropbox_block_path, dropbox_block_name, dropbox_block_pose)
    """
    ...
    j=Pick_Place(0.8,-0.0,0.74)
    d=Pick_Place(0.8,-0.4,0.74)
